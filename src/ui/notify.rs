...
mod platform {
    ...
    fn open_url(url: &str) {
        ...
        match systemd_run_status {
            ...
            Ok(status) => {
                eprintln!(
                    "(ui) systemd-run failed with status {status} for open_url, falling back"
                );
            }
            Err(err) => {
...
mod platform {
    ...
    use winrt_notification::{Duration, LoopableSound, Scenario, Sound, Toast};
    ...
    pub fn send_toast(
        ...
        let mut toast = Toast::new(appname).title(summary);
        ...
        if !remaining.is_empty() {
            toast = toast.text2(&remaining);
        }

        toast = toast.duration(map_duration(timeout));
        if matches!(timeout, ToastTimeout::Never) {
            toast = toast.scenario(Scenario::Reminder);
        }
        toast = toast.sound(map_sound(urgency));

        toast
            .show()
            .with_context(|| "échec d’affichage de la notification Windows")?;

        Ok(())
    }

    fn map_duration(timeout: ToastTimeout) -> Duration {
        match timeout {
            ToastTimeout::Never => Duration::Long,
            ToastTimeout::Milliseconds(ms) if ms >= 7_000 => Duration::Long,
            _ => Duration::Short,
        }
    }

    fn map_sound(urgency: ToastUrgency) -> Option<Sound> {
        match urgency {
            ToastUrgency::Critical => Some(Sound::Loop(LoopableSound::Alarm4)),
...
#[cfg(target_os = "linux")]
pub use platform::{AckControls, send_toast};
#[cfg(target_os = "windows")]
pub use platform::{AckControls, send_toast};
